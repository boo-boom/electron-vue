<template>
  <div class="home">
       <el-form :model="baseTemplate" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
    <div class="layout">
      <div class="title">
        <span>基本设置</span>
        ...
        <el-button type="success" size="mini" @click="generateDoc('ruleForm',baseTemplate.apiName)">生成</el-button>
      </div>
        ...
        <el-row :gutter="20">
          <el-col class="input" :span="12">
            <el-form-item label="安全级别" prop="securityLevel">
              <el-radio border size="mini" v-model="baseTemplate.securityLevel" label="Anonym">Anonym</el-radio>
    ...
    <div class="layout">
      <div class="title">
        <span>请求参数</span>
          <el-dropdown @command="checkBaseTemplate">
            <el-button type="primary" size="mini">
              选择实体<i class="el-icon-arrow-down el-icon--right"></i>
            </el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="chenzhixiang">陈志祥</el-dropdown-item>
              <el-dropdown-item command="chenzhixiang1">狮子头</el-dropdown-item>
        ...
        <el-row :gutter="20" v-for="(param,index) in params" :key="index">
...
export default {
  ...
  data() {
    const verApiName = (rule, value, callback) => {
      ...
      if (!/^node_([a-z]+\.[a-z]+)$/ig.test(value)) {
    ...
    return {
      ...
      rules: {
        apiName: [
          { validator: verApiName, trigger: 'change' },
        ],
        title: [
  ...
  methods: {
    checkBaseTemplate(command) {
      const filePath = resolve(__dirname, `../assets/baseTemplate/${command}.json`)
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if(name) {
            ...
            const filename = resolve(__dirname, `../assets/apiDoc/${_name[1]}.js`);
            fs.writeFile(filename, docStr(this.baseTemplate, this.filterParams(this.params)), (err) => {
              if(err) {
